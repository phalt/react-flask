...
def generate_request_response_classes(
    func: Callable,
) -> Tuple[type, typing.List[Union[Type[APIResponse], Type[None]]]]:
...
def generate_api_decorator(
    ...
    def handle_api_route(**kwargs):
        try:
            ...
            if method == "GET":
                request = get_request_converter.structure(  # type: ignore
                    dict(**kwargs, **flask.request.args), Request
                )
            elif method == "POST":
                request = converter.structure(
                    dict(**kwargs, **flask.request.json), Request
                )
            else:
        ...
        try:
            response = func(**attr.asdict(request, recurse=False))
